...
export function buildFillInstructions(tokens: readonly TokenName[]): string {
  const describe = {
    ...
    scope:
      "one short word naming the concrete area, or - when it adds no information the summary does not",
    summary: "one short imperative line naming the concrete thing changed, no trailing period",
    body: "one short paragraph: what changed and whether behavior changed, or - if nothing more is worth saying",
...
function buildPolicy(template: string): PromptPolicy {
  ...
  const commitSemantics = [
    ...
    "",
    "SUBJECT",
    "The summary must name the concrete thing that changed — the type, function, file, or area — not the category of the change.",
    "A reader of the subject alone should know which part of the codebase this commit touches.",
    "Write it as a changelog bullet: an active verb phrase, no vague verbs like update, improve, or clean up on their own.",
    "Wrap code identifiers — function, type, flag, or file names — in backticks.",
    "Bad: Format type signatures and chained calls. Good: Collapse multiline `FetchFn` and `LoopDeps.startLoader` signatures.",
    "Bad: Update template wording. Good: Require commit subjects to name the concrete change.",
    "",
    "SCOPE",
    "Scope is optional; use - when it adds no information the subject does not already carry.",
    "Omit scope by default; use one only when the project has established scopes.",
    "Never pad the subject with a generic scope like src, core, or misc — a scope that fits any change fits none.",
    "",
    "BODY",
    "Say what changed and whether behavior changed.",
    "For formatting, whitespace, or style-only changes, name the symbols or files touched and state that no behavior changed.",
    "Do not give a reason that is true of every such change — avoid boilerplate like for consistency, for readability, or to clean up.",
    "",
    "COMMIT TYPE",
    "Choose the type that best represents the primary purpose of the change.",
    "The type describes what the change does, not the file it touches — editing strings inside a prompt builder is not docs.",
    "Visible types (feat, fix, perf, revert) mean the change affects users or released behavior.",
    "Hidden types (docs, style, chore, refactor, test, build, ci) mean no user-visible behavior change.",
    "Before choosing a hidden type, ask: does this change what users see or how the product behaves? Prompt text, user-facing messages, defaults, and config values are behavior. If yes, use a visible type.",
